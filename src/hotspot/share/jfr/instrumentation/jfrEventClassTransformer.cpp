...
static const char* utf8_constants[] = {
  ...
  "startTime",    // 11
  "Z",            // 12
  "shouldCommitFld", // 13 // LAST_REQUIRED_UTF8
...
enum utf8_req_symbols {
  ...
  UTF8_REQ_startTime,
  UTF8_REQ_Z_FIELD_DESC,
  UTF8_REQ_shouldCommitFld,
...
static const char context_provider_constant[] = "Ljdk/jfr/experimental/ContextProvider;";
...
static bool is_context_provider_klass(const InstanceKlass* ik) {
  assert(ik != nullptr, "invariant");
  assert(JdkJfrEvent::is_a(ik), "invariant");

  static const Symbol* symbol = nullptr;
  if (symbol == nullptr) {
    symbol = SymbolTable::probe(context_provider_constant, sizeof context_provider_constant - 1);
  }
...
static u2 add_field_infos(JfrBigEndianWriter& writer, const u2* utf8_indexes, bool untypedEventConfiguration, bool is_context_provider) {
  assert(utf8_indexes != nullptr, "invariant");
  ...
  if (is_context_provider) {
    add_field_info(writer,
                   utf8_indexes[UTF8_REQ_shouldCommitFld],
                   utf8_indexes[UTF8_REQ_Z_FIELD_DESC]);
    offset++;
  }
...
static u1* schema_extend_event_subklass_bytes(const InstanceKlass* ik,
  ...
  const u2 number_of_new_fields = add_field_infos(writer, utf8_indexes, untypedEventHandler, is_context_provider_klass(ik));
  assert(writer.is_valid(), "invariant");
